...
LandingServer::LandingServer() : rclcpp::Node("landing_server")
{
  // Declare parameters
  this->declare_parameter<std::vector<std::string>>("drone_ids", std::vector<std::string>{"tello1"});
  this->declare_parameter<std::string>("landing_service_name", "land");
  this->declare_parameter<double>("client_timeout_sec", 5.0);

  // Get parameters
  this->get_parameter("drone_ids", drone_ids_);
  this->get_parameter("landing_service_name", landing_service_name_);
  ...
  // This allows multiple simultaneous calls to /land_all to be processed concurrently
  reentrant_callback_group_ = this->create_callback_group(
      rclcpp::CallbackGroupType::Reentrant);

  // Service server to trigger landing for all drones
  // Use the reentrant callback group to allow parallel execution
  land_all_srv_ = this->create_service<std_srvs::srv::Trigger>(
      "land_all",
      std::bind(&LandingServer::handle_land_all, this, std::placeholders::_1, std::placeholders::_2),
...
void LandingServer::create_service_clients()
{
  ...
  for (const auto &id : drone_ids_)
  {
    ...
    std::string service = "/" + id + "/" + landing_service_name_;
    clients_[id] = this->create_client<std_srvs::srv::Trigger>(service);
  }
}

void LandingServer::print_startup_summary()
{
  if (drone_ids_.empty())
  {
  ...
  else
  {
    ...
    RCLCPP_INFO(this->get_logger(), "Landing Server configured for drones: [%s]", list.str().c_str());
    for (const auto &id : drone_ids_)
    {
      RCLCPP_INFO(this->get_logger(), "Will call service: /%s/%s", id.c_str(), landing_service_name_.c_str());
    }
  }
}

void LandingServer::handle_land_all(
    ...
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  ...
  for (const auto &id : drone_ids_)
  {
    ...
    auto future = call_single_drone_landing_async(id, client);
  ...
  if (futures.empty())
  {
    ...
    RCLCPP_ERROR(this->get_logger(), "No futures created - cannot proceed with landing");
    return;
  }
  ...
  while (rclcpp::ok() && !all_done)
  {
    ...
    {
      RCLCPP_ERROR(this->get_logger(), "Global timeout reached while waiting for all landing responses");
      break;
    }
...
LandingServer::call_single_drone_landing_async(const std::string &id,
